...
async def run_experiment(
    ...
    num_groups: int = 3,
    num_students: int = 30,
    pipeline_model: str = "gemini-2.5-flash",
    ...
    print("\n Initializing MAS Pipeline...")
    pipeline = MASPipeline(gemini_api_key, pipeline_model)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run MAS evaluation experiment with Gemini as judge"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    pipeline_model = get_config_value(config, ["models", "mas", "pipeline"], "gemini-2.5-flash")
    evaluator_model = get_config_value(config, ["models", "mas", "evaluator"], "gemini-2.5-flash")
