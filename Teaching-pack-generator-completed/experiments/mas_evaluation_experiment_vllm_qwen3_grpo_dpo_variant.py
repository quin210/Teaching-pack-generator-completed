"""
MAS Evaluation Experiment (Qwen3-4B SFT-GRPO-DPO Variant via vLLM)
==================================================================
...
Generation: Qwen3-4B with a SFT-GRPO-DPO LoRA adapter in vLLM
Evaluation: Gemini (configurable)

Usage:
    python experiments/mas_evaluation_experiment_vllm_qwen3_grpo_dpo_variant.py --lesson_summary <path> --ground_truth <path>
"""
...
DEFAULT_VLLM_LORA = "qwen3-grpo-dpo-variant"
    ...
    try:
        ...
        if isinstance(data, dict):
            if model_cls is LessonSummary:
                ...
                if "skill_set" in data and isinstance(data["skill_set"], dict):
                    data = data["skill_set"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run MAS evaluation with Qwen3-4B SFT-GRPO-DPO-TAIDUC (vLLM LoRA) and Gemini 2.0 Flash"
    )
    parser.add_argument(
        "--lesson_summary",
        type=str,
